...
- 최종 마감: 2026-09-07. 웹 UI 패치 r32까지를 1.4.4로 닫고, Android 공통 UI 전환 및 이번 세션의 모바일 후속 변경은 `update_1.4.5.md`에서 관리한다.
...
## 추가 패치 - 영상 목록 섹션 구분 (동일 1.4.4-r30)

- 영상 목록에 테마 전경색을 4% 섞은 은은한 배경과 작은 내부 여백을 적용해 별도 사이드 영역으로 구분한다.
- Videos 표시 제목을 제거하고 목록을 위로 당긴다. 접근성 영역 이름 Video list는 유지한다.
- 목록의 스크롤바는 숨기되 기존 스크롤과 무한 로딩, PC 하단 여백은 유지한다.
- 검증/배포: 변경 diff, TypeScript/Vite build와 버전 검증 통과. r30 웹 배포 완료.

## 추가 패치 - 영상·이미지 여백 소폭 축소 (동일 1.4.4-r31)

- 일반 영상 화면의 여백을 sm 24→20px, PC 32→24px로 줄인다. 영상과 목록 사이 간격은 24→20px로 맞춘다.
- 오른쪽 목록 폭은 384→392px로 조금 늘리고 상하 여백 감소분만큼 높이도 늘린다. 영상 높이 계산의 상단 여백도 함께 조정한다.
- 이미지 화면 외곽 여백은 모바일16→12px, sm24→20px로 줄이고 본문 최소 높이에 같은 값을 반영한다.
- 검증/배포: 변경 diff, TypeScript/Vite build 및 버전 검증 통과. r31 웹 배포 완료.

## 추가 패치 - 패딩 2px 축소 및 목록 여백 대칭 (동일 1.4.4-r32)

- 일반 영상 여백은 PC24→22px, sm20→18px로 줄이고 이미지 여백도 모바일12→10px, sm20→18px로 줄인다. 기존 0px 모바일 영상 좌우 여백은 유지한다.
- 영상 목록 폭을392→396px로 늘리고 높이는 동일한 상단 여백 변수를 두 번 빼서 위아래 여백을 맞춘다.
- 영상 페이지 스크롤바를 숨겨 오른쪽에만 추가 폭이 잡히지 않게 한다. 스크롤 동작은 유지하며 PC 영상 왼쪽/목록 오른쪽/목록 위아래 여백을22px로 통일한다.
- 검증/배포: 변경 diff, TypeScript/Vite build, 버전 검증 통과. r32 웹 배포 완료.
