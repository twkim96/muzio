...
## 추가 패치 - 미디어 화면 공통 상단바 유지 (동일 1.4.4-r25)

- 음악 전체 플레이어·영상 시청·이미지 뷰어의 좌측 상단 아래 화살표를 제거하고 PC/모바일 모두 기존 플로팅 상단바를 유지한다. 스와이프 축소 동작은 유지한다.
- 공통 상단바 아래에 미디어 오버레이를 배치하고 남은 뷰포트 높이에 영상·이미지·음악 표시 영역을 맞춘다. 영상의 기존 화살표용 상단 공백을 제거하고 제목/스트림 액션 공간을 계속 확보한다.
- 상단 메뉴 클릭은 해당 라이브러리로 돌아가며, 활성 메뉴는 배경 경로가 아닌 보고 있는 미디어 종류를 기준으로 하나만 표시한다. 직접 플레이어/이미지 URL에서도 상단바와 메뉴/검색 진입을 제공한다.
- 검증/배포: App 27 및 FullPlayerScreen 42 tests 통과, TypeScript/Vite build와 버전/diff 검증 통과. 브라우저 PC 1280px·모바일 390px에서 상단바와 영상 영역 사이 12px 이격, 이미지 뷰어 상단바 유지, 화살표 제거, 메뉴 전환 및 단일 활성 메뉴를 확인했다. 1.4.4-r25 웹 배포 완료.
