...
## 추가 패치 - 미니바 영상 썸네일 (동일 1.4.4-r14)

- 준비된 영상 프레임 썸네일도 playback source의 artworkUrl로 전달해 미니바에 표시한다. 기존 오디오 앨범아트와 이미지 오류 시 기본 아이콘은 유지한다.
- 재생 중인 항목의 썸네일을 직접 구독해, 썸네일 생성 완료 알림만 도착해도 미니바에 즉시 반영한다. 재생 소스 URL은 유지한다.
- 검증: source 15, MiniPlayer 21, App 25 tests 및 TypeScript/Vite build 통과.

## 추가 패치 - 모바일 상단 공간 (동일 1.4.4-r15)

- 768px 미만에서는 상단 타이틀 배지를 숨겨 상단 섬을 한 행에 배치한다. 사이드바 내부 타이틀은 유지한다.
- 480px 미만에서는 상단 설정 버튼도 숨긴다. 설정은 기존 사이드바 메뉴에서 계속 접근할 수 있다.
- 최종 검증/배포: App 25 tests 재통과, TypeScript/Vite build 및 version/diff 검증 통과. 실제 브라우저에서 영상 2개 썸네일 JPEG(320×180) 로드와 새로고침 복원 확인. 320/390px 타이틀·설정 숨김 및 가로 넘침 없음, 492px 설정/섬 간격 확인. 1.4.4-r15 웹 배포 완료.
- 열린 이전 탭의 지연 로딩 모듈 요청도 처리하도록 기존 해시 assets를 배포 디렉터리에 보존했다.

## 추가 패치 - 모바일 글래스바 높이 (동일 1.4.4-r16)

- 640px 미만에서 하단 미니바의 세로 여백을 줄여 기본 높이를 76px에서 68.4px로 10% 축소한다.
- 같은 화면 폭에서 상단 섬은 58px에서 49.3px로 15% 축소하며, 표시되는 설정 버튼의 세로 중앙을 맞춘다. 텍스트와 컨트롤 크기는 유지한다.
- 검증/배포: TypeScript/Vite build, version/diff 검증 통과. 실제 390px 화면에서 상단 49.30px·하단 68.39px 및 가로 넘침 없음 확인. PC 1280px에서는 기존 상단 58px·하단 76px 유지 확인. 1.4.4-r16 웹 배포 완료.

## 추가 패치 - 목록 상단 간격과 검색 글래스 (동일 1.4.4-r17)

- 라이브러리 맨 위 구분선을 제거하고, 비어 있는 보조 헤더를 생략해 정렬 행을 상단 섬 아래 12px로 당긴다. 선택 모드의 작업 버튼은 유지한다.
- 검색창 선택 시 빨간 테두리를 제거한다. 검색 팝업을 상단 섬의 backdrop 영역 밖으로 옮겨 같은 글래스 재질이 페이지 내용을 직접 흐리게 하며, 검색창 내부 클릭과 외부 클릭/ESC 닫기 동작을 유지한다.
- 검증/배포: LibraryScreen 31·App 25 tests 및 TypeScript/Vite build 통과. 실제 PC 1280px/모바일 390px 모두 섬-정렬행 간격 12px, 최상단 border 0px, 검색창 중앙 정렬과 회색 포커스 테두리 확인. 페이지 내용이 검색창 뒤로 흐려지는 글래스 효과와 모바일 검색/초기화 확인. 1.4.4-r17 웹 배포 완료.

## 추가 패치 - 중앙 섬 타이포그래피 (동일 1.4.4-r19)

- 실제 브라우저에서 타이틀과 중앙 탭의 동일한 Pretendard font-family를 확인했다. 타이틀은 20px/600, 중앙 탭은 14px/600이었다.
- 중앙 Music/Video/Image를 16px/500으로 변경한다. 기존 줄 높이를 유지해 상단 섬의 높이는 그대로 둔다.
- 360px 미만에서는 탭 좌우 여백을 2px씩 줄여 커진 텍스트와 검색 버튼이 겹치지 않도록 했다.
- 검증/배포: TypeScript/Vite build 및 version/diff 확인 통과. 실제 PC에서 중앙 탭 16px/500, 모바일 320px에서 버튼 겹침·가로 넘침 없음과 기존 섬 높이 49.3px 유지 확인. 1.4.4-r19 웹 배포 완료.

## 추가 패치 - 상단 섬 글자색 통일 (동일 1.4.4-r20)

- 중앙 Music/Video/Image 중 선택되지 않은 메뉴만 왼쪽 타이틀과 동일한 글래스 텍스트 색상(app-menu-ink)을 사용한다. 선택 메뉴의 기존 강조색과 배경, 16px/500 크기·굵기는 유지한다.
- 검증/배포: TypeScript/Vite build, version/diff 검증 통과. 실제 브라우저에서 비선택 Video/Image와 타이틀의 rgba(210, 211, 214, 0.96) 일치 및 선택 Music의 기존 분홍 강조색 유지 확인. 1.4.4-r20 웹 배포 완료.

## 추가 패치 - 사이드바와 Queue 디자인 정리 (동일 1.4.4-r21)

- 사이드바 기본 플레이리스트를 한글화하고 기본/사용자 목록 및 하단 Setting/Queue에 아이콘을 배치한다. 저장된 사용자 플레이리스트 이름과 항목 ID는 유지한다.
- 사이드바 X를 제거하고 타이틀 클릭/바깥 클릭/ESC 닫기를 유지한다. 작업 버튼은 Edit → + 순서로, 작업 행과 첫 플레이리스트 사이 간격은 목록끼리와 같은 4px로 맞춘다.
- 공통 하트 외곽선을 bold로 강화하고 미니바 오른쪽 좋아요·시계·Queue를 재생 버튼과 같은 선명한 색으로 표시한다. 활성 상태의 강조색은 유지한다.
- Queue의 폭, 위치, 제목 배지, 여백, 글래스 재질과 바깥 배경을 사이드바에 맞춘다. X는 제거하며 바깥 클릭/ESC와 키보드 포커스 이동, 기존 목록 가상화는 유지한다.
- 검증/배포: App 25, smartCollections 3, MiniPlayer 21, QueueDrawer 3 tests와 TypeScript/Vite build, version/diff 검증 통과. 실제 PC/390px 모바일에서 양 패널의 동일 위치·폭·글래스, 한글 아이콘 목록, Edit/+ 순서와 4px 간격, 미니바 오른쪽 3개 아이콘의 흰색 표시, Queue 바깥 클릭/사이드바 타이틀 닫기를 확인했다. Queue 목록 가상화 및 키보드 닫기는 자동 테스트로 검증했다. 1.4.4-r21 웹 배포 완료.

## 추가 패치 - 하트 외곽선 복원 (동일 1.4.4-r22)

- 공통 하트 아이콘의 미선택 외곽선을 기존 light 두께로 되돌린다. 미니바 오른쪽 아이콘의 선명한 색상, 선택된 하트의 채움과 강조색, 사이드바/Queue 변경은 유지한다.
- 검증/배포: 원래 하트 weight로 복원된 diff, TypeScript/Vite build와 버전 검증 통과. 1.4.4-r22 웹 배포 및 서비스 응답 확인 완료.

## 추가 패치 - 플레이리스트 글래스 모달과 모바일 시트 (동일 1.4.4-r23)

| 대상 | PC | 640px 미만 모바일 |
| --- | --- | --- |
| 내비게이션 / Queue / 플레이리스트 목록 | 동일한 왼쪽 글래스 사이드바 | 좌우 20px, 하단 safe-area, 22px 모서리의 최대 60dvh 글래스 시트 |
| 플레이리스트 생성 / 이름 변경 / 항목 추가 | 공통 글래스 중앙 모달 | 공통 글래스 바텀시트 |
| 플레이리스트 및 항목 삭제 확인 | 공통 글래스 확인 모달 | 공통 글래스 확인 시트 |

- ui-kit의 헤더/스크롤 본문/푸터 구조, 낮은 blur와 하이라이트 테두리, 강조색 12% tint 버튼을 적용한다. 흰색으로 꽉 찬 CTA를 제거한다.
- 포털 모달은 테마 변수를 공유하고, 최상단 Escape/Tab 처리와 포커스 복원·스크롤 잠금을 제공한다. 모바일은 입력창을 자동으로 열지 않으며 visualViewport를 따라 키보드 위의 사용 가능한 높이에 맞춘다.
- 모바일 시트가 열린 동안 미니바는 가려져 시트와 겹치지 않으며 재생 상태는 유지한다.
- 검증/배포: App·LibraryScreen·QueueDrawer 관련 60 tests, TypeScript/Vite build와 버전 검증 통과. 브라우저의 PC 및 390px 모바일 뷰포트에서 중앙 모달/하단 시트 위치, 공통 글래스 재질, 플레이리스트 내부 스크롤 영역과 미니바 겹침 방지를 확인했다. 중첩 모달의 Escape·Tab·포커스 복원은 자동 테스트로 검증했다. 실제 모바일 기기의 키보드 동작은 별도 확인이 필요하다. 1.4.4-r23 웹 배포 완료.

## 추가 패치 - 모바일 Queue 표시와 사이드바 스크롤 복구 (동일 1.4.4-r24)

- Edit과 +는 동일한 40px 높이에서 중앙 정렬한다. 글자 +를 SVG 아이콘으로 교체해 폰트 기준선에 따른 높이 차이를 제거한다.
- 내비게이션·Queue·플레이리스트 목록의 기본 스크롤바를 숨기고 내부 스크롤은 유지한다.
- 메뉴에서 연 Queue 및 플레이리스트에는 우측 상단에 설정 버튼과 같은 크기/재질의 원형 뒤로가기를 제공한다. 누르면 이전 내비게이션으로 돌아간다.
- Queue를 body 포털로 옮겨 모바일에서 미니바를 숨길 때 Queue까지 숨겨지거나 미니바의 변형/블러 영역에 갇히는 문제를 고친다.
- 플레이리스트·메뉴 목록과 공통 모달 본문을 스크롤 허용 영역으로 등록해 전역 pull-to-refresh 차단이 목록을 위로 되돌리는 터치까지 막지 않도록 한다.
- 검증/배포: App 26, MiniPlayer 21, LibraryScreen 32, QueueDrawer 5 tests 통과. 숨겨진 미니바 내부 Queue의 포털 표시, 목록 하단에서 되돌리는 touchmove 허용, 뒤로가기 동선 회귀 검증 포함. TypeScript/Vite build, 버전/diff 검사 통과. 브라우저 390px에서 미니바 Queue 표시 및 양 패널 뒤로가기, 스크롤바 숨김, Edit/+ 동일 높이를 확인했고 PC 1280px에서 좌측 8px/폭 320px 사이드바 유지를 확인했다. 실제 모바일 기기의 터치 조작은 별도 확인이 필요하다. r24 웹 배포 완료.

## 추가 패치 - 상단바 유지 롤백 및 화살표 공간 제거 (동일 1.4.4-r26)

- r25 공통 상단바 유지 변경을 git revert로 되돌린다. 음악·영상·이미지 뷰어는 이전처럼 화면 전체 영역을 사용한다.
- 세 미디어 화면의 아래 화살표만 제거한다. 영상의 상단 56px/64px 공간과 이미지 본문의 상단 48px 패딩을 없애 콘텐츠를 위로 올린다. 기존 비율 유지/영상 제목 공간 및 스와이프 축소 동작은 유지한다.
- 검증/배포: App 26 및 FullPlayerScreen 42 tests, TypeScript/Vite build, 버전/diff 검증 통과. 닫기 버튼 대신 음악·이미지·영상 스와이프 축소를 기존 테스트로 확인했다. 브라우저 PC/모바일 크기에서 전체 화면 오버레이가 상단 0px부터 시작하고 화살표가 없는 것을 확인했다. r26 웹 배포 완료.

## 추가 패치 - 영상 상단·좌우 여백 일치 (동일 1.4.4-r27)

- 일반 영상 화면의 상단 여백을 기존 좌우 여백에 맞춰 sm에서 24px, lg에서 32px로 적용한다. 좌우 여백이 없는 작은 모바일 및 영화관 모드는 기존 0px를 유지한다.
- 영상 최대 높이 계산에도 같은 상단 여백을 반영해 제목과 스트림 액션 공간을 유지한다.
- 검증/배포: 변경 diff, TypeScript/Vite build, 버전 검증 통과. r27 웹 배포 완료.

## 추가 패치 - Videos 목록 하단 높이 맞춤 (동일 1.4.4-r28)

- PC 일반 영상 화면의 Videos 영역을 뷰포트에서 상단·하단 여백을 뺀 높이로 맞춘다. 제목을 제외한 나머지를 목록 스크롤 영역이 채워 하단에도 좌우와 같은 32px 여백만 남긴다.
- 기존 고정 7rem 차감 최대 높이를 제거하고 flex 본문으로 전환한다. 모바일과 영화관 모드의 목록 배치는 유지한다.
- 검증/배포: 변경 diff, TypeScript/Vite build, 버전 검증 통과. r28 웹 배포 완료.

## 추가 패치 - 위치명 정리 및 글래스 축소 버튼 (동일 1.4.4-r29)

- 영상 제목 아래 파일 위치명 줄을 제거한다. 극장모드에서 확보하는 요약 높이는 제목/스트림 버튼의 실제 높이를 측정하므로 제거된 줄만큼 영상 표시 영역이 늘어난다.
- 음악·영상·이미지 좌측 상단에 설정 버튼과 같은 46.4px 원형 글래스 축소 버튼을 복원한다. 화면 위에 떠 있으므로 콘텐츠 여백은 추가하지 않는다.
- 표준/WebKit 전체화면 진입 시 버튼을 숨기고 해제 시 복원한다. 기존 축소 콜백과 스와이프는 유지한다.
- 검증/배포: App 26, FullPlayerScreen 42, MediaCollapseButton 1 tests 통과. 전체화면 숨김/복원 및 클릭 회귀 확인 포함. TypeScript/Vite build, 버전/diff 검증 통과. 브라우저 모바일390px/PC1280px에서 버튼46.4px·원형·글래스 필터, 위치명 제거, 클릭 축소를 확인했다. r29 웹 배포 완료.
