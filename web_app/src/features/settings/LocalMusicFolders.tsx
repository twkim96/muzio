...
export function LocalMusicFolders({ query }: { query: string }) {
  ...
  const platform = androidShellBridge()?.platform;
  ...
  return <section id="local-music-folders" hidden={!matches} className="min-w-0 border-t border-zinc-200/70 py-6 dark:border-white/10">
    <div className="mb-5 flex flex-wrap items-start justify-between gap-4">
      <div><h2 className="text-2xl font-semibold">Local Music</h2><p className="mt-1 text-sm text-muted">이 기기의 음악 폴더를 추가합니다. 네트워크 음악과 함께 표시됩니다. {apple ? '기기에 내려받은 파일을 재생할 수 있습니다. 앱 화면을 처음 열 때는 서버 연결이 필요합니다.' : '오프라인에서도 재생할 수 있습니다.'}</p></div>
    ...
    {!busy && enrichment?.scanning && <p role="status" className="mb-2 text-sm text-muted">폴더 목록을 불러오는 중…</p>}
    ...
    <p className="mt-3 text-xs text-muted">음악 파일은 복사하지 않습니다. 곡 정보는 차례로 채우고, 앱을 다시 열면 남은 작업을 이어갑니다. 폴더를 제거해도 원본 파일은 삭제되지 않습니다.{apple && ' iCloud 등 온라인 폴더의 파일은 먼저 기기에 다운로드해 주세요. 앱이 정지되면 정보 수집은 다음 실행 때 이어집니다.'}</p>
  </section>;
}
