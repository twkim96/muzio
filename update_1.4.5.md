...
- 상태: 개발 브랜치 구현·자동 검증·에뮬레이터 동작 검증 완료. Samsung versionCode11 설치/실행 및 운영 웹 배포 완료. 최신 versionCode13은 에뮬레이터 검증/APK 생성 완료, 사용자 휴대폰 설치는 퇴근 후 연결 시 진행. 실제 개인 라이브러리 사용성 검증은 별도.
...
- 현재 웹 배포: PWA `1.4.5-r8`. 최신 Android 산출물: versionCode 13. 실제 Samsung 설치 확인은 versionCode 11까지이며, 최신 APK 설치는 사용자가 퇴근 후 기기를 연결하면 진행한다.
