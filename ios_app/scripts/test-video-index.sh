...
root=pathlib.Path(sys.argv[1]); size=20*1024*1024
class Handler(http.server.BaseHTTPRequestHandler):
 ...
 def serve(self,head):
  ...
  if ident=='race' and 'index_revision' in query and self.headers.get('Range','').startswith('bytes=16778240-'):
   self.send_response(409);self.send_header('Content-Length','0');self.end_headers();return
  if ident=='badstartup' and 'index_revision' in query:revision='f'*64
  ...
  try:
   ...
   if ident=='slow':(root/'canceled').write_text('yes')
